...
Keep the original No-Intro collection intact. Do not rename, delete, or flatten variants merely to simplify the UI. RomM scans hashes and metadata, while the configured region preference helps choose a representative result. The Compose stack mounts `games/roms` directly as RomM's library root, so platform folders begin immediately beneath it. Adjust `system.platforms` in `config/config.yml` only when your existing folder names differ from RomM's platform slugs; it maps names without moving files.
